...
const SimulationOverviewModal = ({ sim, onClose }: { sim: SimulationData; onClose: () => void }) => {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-[#111827] rounded-2xl border border-[#1e293b] shadow-2xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="p-6 border-b border-[#1e293b] flex justify-between items-start">
          <div>
            <h3 className="text-lg font-bold text-white">{sim.name}</h3>
            <p className="text-sm text-slate-400 mt-1">{sim.description || 'No description provided.'}</p>
          </div>
          <button onClick={onClose} className="p-1 text-slate-400 hover:text-white rounded-lg flex-shrink-0"><X className="w-5 h-5" /></button>
        </div>

        <div className="p-6 space-y-4">
          {/* Quick stats */}
          <div className="grid grid-cols-3 gap-3">
            <div className="bg-[#172033] p-3 rounded-lg border border-[#1e293b] text-center">
              <p className="text-2xl font-bold text-white">{participantCount}</p>
              <p className="text-[10px] text-slate-400 uppercase font-semibold mt-1">Participants</p>
            </div>
            <div className="bg-[#172033] p-3 rounded-lg border border-[#1e293b] text-center">
              <p className="text-2xl font-bold text-white">—</p>
              <p className="text-[10px] text-slate-400 uppercase font-semibold mt-1">Total Trades</p>
            </div>
            <div className="bg-[#172033] p-3 rounded-lg border border-[#1e293b] text-center">
              <p className="text-2xl font-bold text-white">—</p>
              <p className="text-[10px] text-slate-400 uppercase font-semibold mt-1">Avg Return</p>
            </div>
          </div>
        ...
        <div className="p-6 border-t border-[#1e293b] flex justify-end">
          <button onClick={onClose} className="px-5 py-2.5 text-sm font-medium text-slate-300 bg-[#172033] border border-[#1e293b] rounded-lg hover:bg-[#1e293b] transition-colors">Close</button>
        </div>
      </div>
    </div>
  );
...
export const AdminSimulations = () => {
  ...
  const getStatusBadge = (status: string) => {
    ...
    if (s === 'active' || s === 'live') return 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20';
    if (s === 'upcoming' || s === 'draft') return 'bg-blue-500/10 text-blue-400 border-blue-500/20';
    if (s === 'completed') return 'bg-slate-500/10 text-slate-400 border-slate-500/20';
    if (s === 'ended') return 'bg-rose-500/10 text-rose-400 border-rose-500/20';
  ...
  if (error && !loading && simulations.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] text-slate-400 gap-4">
        ...
        <h2 className="text-xl font-bold text-white">Unable to load simulations</h2>
        <p>{error}</p>
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-10">
      <div>
        <h1 className="text-3xl font-bold text-white tracking-tight">Simulation Management</h1>
        <p className="text-slate-400 mt-2 text-lg">Monitor and manage trading simulations across the platform.</p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <div key={i} className="bg-[#111827] p-4 rounded-xl border border-[#1e293b] shadow-lg">
            <p className="text-xs font-medium text-slate-400 uppercase tracking-wider">{card.label}</p>
            <h3 className={`text-2xl font-bold mt-1 text-${card.color === 'slate' ? 'white' : card.color + '-400'}`}>
              {loading ? <span className="inline-block w-8 h-6 bg-[#1e293b] rounded animate-pulse"></span> : card.value}
            </h3>
          </div>
        ))}
      </div>
      ...
      <div className="bg-[#111827] rounded-xl border border-[#1e293b] shadow-lg">
        <div className="p-4 flex flex-col lg:flex-row gap-3 items-start lg:items-center justify-between">
          <div className="relative w-full lg:max-w-sm">
            ...
            <input type="text" placeholder="Search simulations..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
          ...
          <div className="flex flex-wrap items-center gap-2">
            <div className="flex items-center gap-1.5 bg-[#172033] border border-[#1e293b] rounded-lg px-3 py-2">
              ...
              <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="bg-transparent border-none text-sm text-slate-300 outline-none cursor-pointer">
            ...
            <tbody className="divide-y divide-[#1e293b]">
              {loading ? (
                Array.from({ length: 4 }).map((_, i) => (
                  <tr key={i}><td colSpan={8} className="px-5 py-4"><div className="h-6 bg-[#1e293b] rounded animate-pulse"></div></td></tr>
                ))
              ) : filtered.length === 0 ? (
                <tr>
                  <td colSpan={8} className="px-5 py-16 text-center">
                    <div className="flex flex-col items-center gap-2 text-slate-500">
                      ...
                      <p className="font-medium">No simulations found</p>
                ...
                filtered.map(sim => (
                  <tr key={sim._id} className="hover:bg-[#172033]/50 transition-colors">
                    ...
                    <td className="px-5 py-3.5 text-slate-400 text-xs">{sim.createdBy?.name || sim.createdBy?.email || '—'}</td>
                    <td className="px-5 py-3.5">
                      <span className="text-xs font-medium text-slate-300 bg-[#172033] px-2 py-0.5 rounded border border-[#1e293b]">{sim.market || '—'}</span>
                    </td>
                    <td className="px-5 py-3.5 text-center">
                      <span className="text-sm font-medium text-white">{sim.participants?.length || 0}</span>
                    </td>
                    <td className="px-5 py-3.5 text-slate-400 text-xs">{sim.startDate ? new Date(sim.startDate).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' }) : '—'}</td>
                    <td className="px-5 py-3.5 text-slate-400 text-xs">{sim.endDate ? new Date(sim.endDate).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' }) : '—'}</td>
                    <td className="px-5 py-3.5">
                    ...
                    <td className="px-5 py-3.5 text-right">
                      <div className="relative inline-block" ref={openMenuId === sim._id ? menuRef : null}>
                        <button onClick={() => setOpenMenuId(openMenuId === sim._id ? null : sim._id)} className="p-1.5 text-slate-400 hover:text-white hover:bg-[#172033] rounded-lg transition-colors">
                          <MoreVertical className="w-4 h-4" />
                        </button>
                        {openMenuId === sim._id && (
                          <div className="absolute right-0 mt-1 w-48 bg-[#172033] rounded-lg shadow-2xl border border-[#1e293b] overflow-hidden z-40">
                            <button onClick={() => { setOverviewSim(sim); setOpenMenuId(null); }} className="w-full flex items-center gap-2.5 px-3 py-2.5 text-sm text-slate-300 hover:text-white hover:bg-white/5 transition-colors">
                              <Eye className="w-4 h-4 text-blue-400" /> View Overview
                            </button>
                            <button onClick={() => setOpenMenuId(null)} className="w-full flex items-center gap-2.5 px-3 py-2.5 text-sm text-slate-300 hover:text-white hover:bg-white/5 transition-colors">
                              <Trophy className="w-4 h-4 text-amber-400" /> View Leaderboard
                            </button>
                            <div className="border-t border-[#1e293b]">
                              <button onClick={() => setOpenMenuId(null)} className="w-full flex items-center gap-2.5 px-3 py-2.5 text-sm text-rose-400 hover:bg-rose-500/10 transition-colors">
