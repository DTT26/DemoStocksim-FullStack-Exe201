...
export const SubmissionListModal = ({ isOpen, onClose, assignment }: SubmissionListModalProps) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-slate-900/60 dark:bg-black/85 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white dark:bg-[#09090b] rounded-2xl shadow-2xl w-full max-w-5xl overflow-hidden flex flex-col max-h-[92vh] border border-slate-200 dark:border-[#262626] text-slate-800 dark:text-slate-200 transition-colors">
        ...
        <div className="px-5 sm:px-6 py-4 sm:py-5 border-b border-slate-200 dark:border-[#262626] flex justify-between items-center bg-slate-50/80 dark:bg-[#000000]">
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
              Tổng số bài nộp: <strong className="text-slate-900 dark:text-white font-mono">{totalSubs}</strong> • Đã chấm điểm: <strong className="text-emerald-600 dark:text-emerald-400 font-mono">{gradedSubs}</strong> / {totalSubs}
            </p>
          </div>
          <button 
            onClick={onClose} 
        ...
        {toastMsg && (
          <div className={`mx-6 mt-4 p-3 rounded-xl text-xs font-semibold flex items-center justify-between border ${
            ...
            <button onClick={() => setToastMsg(null)} className="text-slate-400 hover:text-slate-700 dark:hover:text-white ml-2 cursor-pointer">✕</button>
          </div>
        )}

        {/* Content list */}
        <div className="p-4 sm:p-6 overflow-y-auto flex-1 space-y-6 bg-white dark:bg-[#09090b]">
          {loading ? (
            ...
            <div className="py-20 text-center text-slate-400">
              ...
              <h4 className="text-base font-bold text-slate-700 dark:text-slate-300">Chưa có sinh viên nào nộp bài</h4>
              ...
              return (
                <div 
                  ...
                  className={`p-4 sm:p-6 rounded-2xl border transition-all ${
                    sub.status === 'GRADED'
                      ? 'bg-purple-50/30 dark:bg-[#14121a] border-purple-200 dark:border-purple-500/40'
                      : 'bg-white dark:bg-[#121214] border-slate-200 dark:border-[#262626] shadow-xs'
                  }`}
                >
                  {/* Top Bar: Student info & Submission status */}
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 pb-4 border-b border-slate-200 dark:border-white/5">
                    <div className="flex items-center gap-3">
                      {sub.studentId?.picture ? (
                        <img src={sub.studentId.picture} alt="" className="w-11 h-11 rounded-full object-cover border border-slate-200 dark:border-white/10" />
                      ) : (
                        <div className="w-11 h-11 rounded-full bg-indigo-50 dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-400 flex items-center justify-center font-bold text-base border border-indigo-200 dark:border-indigo-500/30">
                          {sub.studentId?.name?.charAt(0) || 'S'}
                        </div>
                      )}
                      <div>
                        <div className="flex items-center gap-2">
                          <h4 className="text-base font-bold text-slate-900 dark:text-white">{sub.studentId?.name || 'Sinh viên'}</h4>
                          {isVerified ? (
                            <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-emerald-50 dark:bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-500/30 flex items-center gap-1">
                              <ShieldCheck className="w-3 h-3" /> Đã thực hành thật
                            </span>
                          ) : (
                            <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-amber-50 dark:bg-amber-500/15 text-amber-600 dark:text-amber-400 border border-amber-200 dark:border-amber-500/30 flex items-center gap-1">
                              <AlertTriangle className="w-3 h-3" /> Chưa phát hiện lệnh
                            </span>
                          )}
                        </div>
                        <span className="text-xs text-slate-500 dark:text-slate-400 font-mono">{sub.studentId?.email}</span>
                      </div>
                    </div>

                    <div className="flex items-center gap-4">
                      <div className="text-right text-xs">
                        <span className="text-slate-400 dark:text-slate-500 block text-[10px]">Thời gian nộp:</span>
                        <span className="text-slate-700 dark:text-slate-200 font-mono font-medium">{new Date(sub.submittedAt).toLocaleString('vi-VN')}</span>
                      </div>

                      {sub.status === 'GRADED' ? (
                        <div className="px-4 py-2 rounded-xl bg-purple-50 dark:bg-purple-500/20 border border-purple-200 dark:border-purple-500/40 text-purple-700 dark:text-purple-300 font-mono font-black text-base shadow-xs">
                          {sub.score} / 100
                        </div>
                      ) : (
                        <span className="px-3.5 py-1.5 rounded-full text-xs font-bold uppercase bg-amber-50 dark:bg-amber-500/15 text-amber-600 dark:text-amber-400 border border-amber-200 dark:border-amber-500/30">
                          Chưa chấm
                        </span>
                      )}
                    </div>
                  ...
                  <div className="py-4 space-y-4">
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center justify-between text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-slate-400">
                        ...
                        <span className="text-[11px] font-mono text-cyan-600 dark:text-cyan-400 normal-case font-semibold">
                          Checklist: {completedCount} / {totalReqs} tiêu chí đạt
                        </span>
                      </div>

                      {/* Checklist breakdown */}
                      <div className="bg-slate-50 dark:bg-[#09090b] p-3.5 rounded-xl border border-slate-200 dark:border-white/5 space-y-2">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
                          {assignment.requirements && assignment.requirements.length > 0 ? (
                            assignment.requirements.map((req: any, i: number) => {
                              ...
                              return (
                                <div key={req.id || i} className="flex items-center gap-2">
                                  ...
                                  <span className={isDone ? 'text-slate-800 dark:text-slate-200' : 'text-slate-400 dark:text-slate-500 line-through'}>
                                    {req.text}
                                  </span>
                                </div>
                              );
                            ...
                            <span className="text-slate-400 dark:text-slate-500 italic text-xs">Không có tiêu chí checklist cụ thể</span>
                          )}
                        </div>
                      </div>

                      {/* Bài phân tích */}
                      <div className="bg-slate-50 dark:bg-[#09090b] p-3.5 rounded-xl border border-slate-200 dark:border-white/5">
                    ...
                    <div className="bg-slate-50 dark:bg-[#121214] rounded-2xl border border-slate-200 dark:border-cyan-500/20 p-4 space-y-3">
                      <div className="flex flex-wrap items-center justify-between gap-2 pb-2.5 border-b border-slate-200 dark:border-white/5">
                      ...
                      {isVerified ? (
                        <>
                          ...
                          {(() => {
                            ...
                            return (
                              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 text-xs">
                                <div className="bg-white dark:bg-[#141a29] p-3 rounded-xl border border-slate-200 dark:border-white/5 shadow-xs">
                                  <span className="text-[10px] text-slate-500 dark:text-slate-400 block uppercase font-bold">Lệnh khớp (Filled)</span>
                                  <span className="text-slate-900 dark:text-white font-mono font-bold text-base mt-0.5 block">{totalFilled} lệnh</span>
                                  {totalCancelled > 0 && (
                                    <span className="text-[10px] text-slate-400 dark:text-slate-500 font-mono block mt-0.5">({totalCancelled} lệnh đã hủy)</span>
                                  )}
                                </div>
                                <div className="bg-white dark:bg-[#141a29] p-3 rounded-xl border border-slate-200 dark:border-white/5 shadow-xs">
                                  <span className="text-[10px] text-slate-500 dark:text-slate-400 block uppercase font-bold">Mã cổ phiếu</span>
                                  <span className="text-cyan-600 dark:text-cyan-400 font-mono font-bold text-base mt-0.5 block">{evidence.targetSymbol || assignment.symbol}</span>
                                </div>
                                <div className="bg-white dark:bg-[#141a29] p-3 rounded-xl border border-slate-200 dark:border-white/5 shadow-xs">
                                  <span className="text-[10px] text-slate-500 dark:text-slate-400 block uppercase font-bold">Kỷ luật Stop Loss</span>
                                  <span className={`font-bold text-sm mt-0.5 block ${evidence.hasStopLoss ? 'text-emerald-600 dark:text-emerald-400' : 'text-amber-600 dark:text-amber-400'}`}>
                                    {evidence.hasStopLoss ? '✓ Có cài SL' : '⚠️ Chưa cài SL'}
                                  </span>
                                </div>
                                <div className="bg-white dark:bg-[#141a29] p-3 rounded-xl border border-slate-200 dark:border-white/5 shadow-xs">
                                  <span className="text-[10px] text-slate-500 dark:text-slate-400 block uppercase font-bold">P&L thực tế</span>
                                  <span className={`font-mono font-bold text-base mt-0.5 block ${evidence.totalPnL >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}>
                          ...
                          {/* Orders table */}
                          <div className="overflow-x-auto rounded-xl border border-slate-200 dark:border-white/5 bg-white dark:bg-transparent">
                            <table className="w-full text-left text-[11px] font-mono">
                              ...
                              <tbody className="divide-y divide-slate-200 dark:divide-white/5 text-slate-700 dark:text-slate-300">
                                {evidence.orders.map((ord: any, idx: number) => {
                                  ...
                                  return (
                                    <tr key={ord.id || idx} className={`transition-colors ${isCancelled ? 'opacity-60 bg-slate-50/50 dark:bg-white/[0.01]' : 'hover:bg-slate-50 dark:hover:bg-white/[0.02]'}`}>
                                      <td className="py-2.5 px-3">
                                        <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                                          isCancelled 
                                            ? 'bg-slate-200 dark:bg-slate-700/50 text-slate-600 dark:text-slate-400 border border-slate-300 dark:border-slate-600'
                                            : isBuy 
                                      ...
                                      <td className="py-2.5 px-3 font-bold text-slate-900 dark:text-white">{ord.symbol}</td>
                                      ...
                                      <td className="py-2.5 px-3">
                                        {ord.pnl !== undefined && !isCancelled ? (
                                          <span className={ord.pnl >= 0 ? 'text-emerald-600 dark:text-emerald-400 font-bold' : 'text-rose-600 dark:text-rose-400 font-bold'}>
                                      ...
                                      <td className="py-2.5 px-3 text-slate-500 dark:text-slate-400 text-[10px]">
                    ...
                    {sub.feedback && !isEditing && (
                      <div className="bg-purple-50 dark:bg-purple-950/20 p-3.5 rounded-xl border border-purple-200 dark:border-purple-500/30 text-xs">
                        <span className="text-purple-700 dark:text-purple-300 font-bold block mb-1">Lời nhận xét của giảng viên:</span>
                        <p className="text-purple-900 dark:text-purple-200 whitespace-pre-wrap leading-relaxed">
                          {sub.feedback}
                        </p>
                      </div>
                    )}
                  ...
                  {isEditing ? (
                    <div className="mt-4 p-4 bg-slate-50 dark:bg-[#10141f] rounded-2xl border border-indigo-200 dark:border-indigo-500/40 space-y-3 animate-in fade-in duration-200">
                      ...
                      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                        <div className="w-full sm:w-36">
                          <label className="block text-[11px] font-semibold text-slate-700 dark:text-slate-300 mb-1">Điểm số (0-100):</label>
                          <input
                            type="number"
                            min={0}
                            max={100}
                            ...
                            className="w-full px-3 py-2 bg-white dark:bg-[#172033] border border-slate-200 dark:border-[#253047] rounded-xl text-slate-900 dark:text-white font-mono text-base font-bold focus:outline-none focus:border-indigo-500"
                          />
                        </div>
                        <div className="flex-1">
                          <label className="block text-[11px] font-semibold text-slate-700 dark:text-slate-300 mb-1">Lời nhận xét / Đánh giá chi tiết:</label>
                          <input
                            type="text"
                            value={feedbackInput}
                            onChange={(e) => setFeedbackInput(e.target.value)}
                            placeholder="Ví dụ: Phân tích đúng chỉ báo, có kỷ luật SL/TP theo yêu cầu đề bài..."
                            className="w-full px-3 py-2 bg-white dark:bg-[#172033] border border-slate-200 dark:border-[#253047] rounded-xl text-slate-900 dark:text-white text-xs focus:outline-none focus:border-indigo-500"
                          />
                        </div>
                      </div>
                      ...
                      <div className="flex justify-end gap-2 text-xs pt-1">
                        <button
                          ...
                          className="px-4 py-2 bg-white dark:bg-white/5 hover:bg-slate-100 dark:hover:bg-white/10 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-transparent rounded-xl font-medium transition-colors cursor-pointer"
                        >
                          Hủy
                        </button>
                        <button
                          ...
                          className="px-5 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl font-bold shadow-sm transition-all flex items-center gap-1.5 cursor-pointer"
                        >
                    ...
                    <div className="pt-2 flex justify-end">
                      <button
                        ...
                        className="px-4 py-2 bg-indigo-50 dark:bg-indigo-600/20 hover:bg-indigo-100 dark:hover:bg-indigo-600/30 text-indigo-600 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-500/30 rounded-xl text-xs font-bold transition-colors flex items-center gap-2 cursor-pointer shadow-xs"
                      >
                        <Award className="w-4 h-4" />
        ...
        {/* Footer */}
        <div className="px-5 sm:px-6 py-4 border-t border-slate-200 dark:border-[#262626] bg-slate-50/80 dark:bg-[#000000] flex justify-end">
          <button
            onClick={onClose}
            className="px-5 py-2.5 bg-slate-200 hover:bg-slate-300 dark:bg-white/10 dark:hover:bg-white/15 text-slate-800 dark:text-slate-200 rounded-xl text-xs font-semibold transition-colors cursor-pointer"
          >
            Đóng
          </button>
        </div>
