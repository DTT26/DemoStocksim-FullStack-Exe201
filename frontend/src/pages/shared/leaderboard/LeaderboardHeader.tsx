...
export const LeaderboardHeader = ({ simulation }: LeaderboardHeaderProps) => {
  ...
  return (
    <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 w-full animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div>
        <div className="flex items-center text-sm text-slate-500 dark:text-[#787b86] mb-3">
          <span className="hover:text-blue-500 cursor-pointer transition-colors">Dashboard</span>
          <ChevronRight className="w-4 h-4 mx-1" />
          <span className="text-slate-900 dark:text-white font-medium">Leaderboard</span>
        </div>
        
        <div className="flex items-center gap-3 mb-2">
          <h1 className="text-3xl md:text-4xl font-bold text-slate-900 dark:text-white tracking-tight">{simulation.name}</h1>
        ...
        <p className="text-slate-600 dark:text-[#787b86] text-lg mb-4">Compete, learn, and improve your trading performance.</p>
        
        <div className="flex flex-wrap items-center gap-2 text-sm text-slate-600 dark:text-slate-300">
          <span className="font-semibold text-slate-900 dark:text-white">{simulation.participants} Participants</span>
          <span className="text-slate-400 dark:text-[#787b86]">•</span>
      ...
      <div className="bg-white dark:bg-[#1e222d] border border-slate-200 dark:border-[#2a2e39] rounded-xl p-4 min-w-[280px] shadow-sm flex flex-col justify-center h-[88px] transition-colors">
        <div className="flex items-center gap-2 text-slate-500 dark:text-[#787b86] mb-1.5 text-sm font-medium">
          <Clock className="w-4 h-4" />
          {simulation.status === 'LIVE' ? 'Simulation ends in' : simulation.status === 'UPCOMING' ? 'Simulation starts in' : 'Status'}
        </div>
        <p className={`text-lg font-mono font-bold ${simulation.status === 'LIVE' ? 'text-slate-900 dark:text-white' : 'text-slate-500 dark:text-slate-400'}`}>
          {timeLeft}
        </p>
      </div>
    </div>
